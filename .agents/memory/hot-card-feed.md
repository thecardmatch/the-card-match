...
**How to apply:** Keep `/api/deck`, `/api/feed`, and swipe persistence compatible with the existing card UI, but do not use swipe events or learned weights to select or reorder cards. Query auctions with ending-soonest ordering, treat bids as a ranking signal rather than an eligibility gate because Browse may omit counters, pass page offsets through to Browse, and let the client append the next page when fewer than five cards remain.