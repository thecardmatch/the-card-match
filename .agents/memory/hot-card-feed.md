...
**How to apply:** Keep `/api/deck`, `/api/feed`, and swipe persistence compatible with the existing card UI, but do not use swipe events or learned weights to select or reorder cards. Apply the strict search/query requirements to every server-side Browse search, query a small set of high-end terms individually because Browse does not support OR expressions, stop immediately on 429s, require at least one high-end signal before admission, use `(bidCount * 3) + (watchCount * 2)` plus PSA 10/1:1/RPA bonuses for local ranking, and pass current-session swiped IDs to the server before replenishing.