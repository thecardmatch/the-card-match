...
**How to apply:** Keep `/api/deck`, `/api/feed`, and swipe persistence compatible with the existing card UI, but do not use swipe events or learned weights to select or reorder cards. Apply the strict search/query requirements to every server-side Browse search, query a small set of high-end terms individually because Browse does not support OR expressions, use `buyingOptions:{AUCTION}` for Ending Soonest and `buyingOptions:{FIXED_PRICE}` for Buy It Now, stop immediately on 429s, require at least one high-end signal before admission, sort auction results by `endTime` ascending, use quality as a tie-breaker, and pass current-session swiped IDs to the server before replenishing.