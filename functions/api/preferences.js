...
export async function onRequestGet({ env, request }) {
  ...
  if (auth.guest) return jsonResponse({ authenticated: false, guest: true, preferences: {} });
  ...
  const [{ data: legacy, error }, { data: canonical, error: canonicalError }] = await Promise.all([
    auth.client.from("user_quiz_results").select("preferences,updated_at").eq("user_id", auth.userId).maybeSingle(),
    auth.client.from("user_preferences").select("preferences,updated_at").eq("user_id", auth.userId).maybeSingle(),
  ]);
  ...
  return jsonResponse({ authenticated: true, preferences: current?.preferences || {} });
}
