...
export default function App() {
  ...
  const displayQuery = prefs ? `${prefs.category} ${prefs.grade || ""}`.trim() : "Loading...";
      ...
      <main className="flex-1 flex flex-col min-w-0 h-full relative overflow-hidden bg-white">
        ...
        <header className="px-4 py-3 border-b flex items-center justify-between bg-white z-50 shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 bg-primary/10 rounded-lg flex items-center justify-center font-black text-primary text-xl">C</div>
            <div className="flex flex-col">
              <h1 className="text-sm font-black uppercase tracking-tighter leading-none">The Card Match</h1>
              <p className="text-[10px] text-primary font-bold uppercase mt-1">{loading ? "Searching..." : displayQuery}</p>
            </div>
          </div>

          <div className="flex items-center gap-2">
            <button className="w-10 h-10 rounded-full border flex items-center justify-center bg-white active:bg-muted"><ArrowUpDown className="w-4 h-4" /></button>
            ...
            <button onClick={(e) => { e.stopPropagation(); setSettingsOpen(true); }} className="w-10 h-10 rounded-full border border-border flex items-center justify-center bg-card hover:bg-muted cursor-pointer relative z-[60]">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-4 relative min-h-0">
          <div className="w-full max-w-[360px] md:max-w-[420px] aspect-[2.8/4] relative z-10 pointer-events-auto">
            <SwipeDeck
              cards={cards}
              onLike={handleLike}
          ...
          <div className="mt-6 flex flex-col items-center gap-3 shrink-0 z-20">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/30 select-none">Swipe Up to Buy</span>
          </div>
        </div>
      </main>

      <aside className="hidden md:flex w-80 h-full bg-white border-l flex flex-col overflow-hidden">
        <div className="p-5 border-b font-black text-xs uppercase tracking-widest bg-white">Watchlist ({liked.length})</div>
        ...
        {watchlistOpen && (
          <>
            ...
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="fixed inset-y-0 right-0 w-[85%] bg-white z-[110] md:hidden flex flex-col shadow-2xl">
              <div className="p-4 border-b flex justify-between items-center font-black text-xs uppercase tracking-widest bg-white sticky top-0">
