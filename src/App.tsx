...
export default function App() {
  ...
  // Main Card Fetching Logic (The Engine)
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
  ...
  // Logic to handle button clicks as manual swipes
  ...
  return (
    <div className="h-screen w-full bg-background flex flex-col md:flex-row overflow-hidden font-sans">
      <main className="flex-1 flex flex-col min-w-0 h-full relative overflow-hidden bg-[#fdfdfd]">
        ...
        {/* HEADER: Z-50 ensures it stays above the cards */}
        <header className="px-4 md:px-6 py-4 border-b border-border flex items-center justify-between bg-background z-50 shrink-0">
          <div className="flex items-center gap-3">
            <img src="/logo.png" alt="Logo" className="w-10 h-10 rounded-lg shadow-sm" />
            <div className="flex flex-col">
              <h1 className="text-sm md:text-lg font-black leading-none tracking-tight uppercase">The Card Match</h1>
          ...
          <div className="flex items-center gap-2">
            ...
            {/* SETTINGS BUTTON: Explicit Z-Index to prevent card blocking */}
            <button 
              onClick={(e) => { e.stopPropagation(); setSettingsOpen(true); }} 
              className="w-10 h-10 rounded-full border border-border flex items-center justify-center bg-card hover:bg-muted transition-colors relative z-[60]"
            >
        ...
        {/* SWIPE DECK AREA: Fixed scaling for Web vs Mobile */}
        <div className="flex-1 flex flex-col items-center justify-center p-4 md:p-12 relative overflow-hidden">
          <div className="w-full max-w-[380px] md:max-w-[440px] h-full max-h-[580px] md:max-h-[720px] relative z-10 pointer-events-auto">
            <SwipeDeck
              cards={cards}
              onLike={handleLike}
          ...
          {/* ACTION BUTTONS */}
          <div className="mt-8 md:mt-12 flex flex-col items-center gap-4 shrink-0 z-20">
            ...
            <span className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/30 select-none">
              Swipe Up to Buy
            </span>
          </div>
        </div>
      ...
      <aside className="hidden md:flex w-80 h-full bg-card border-l border-border flex flex-col overflow-hidden">
        <div className="p-5 border-b border-border flex items-center justify-between bg-background/50 backdrop-blur">
          <h2 className="font-black uppercase tracking-tighter text-xs flex items-center gap-2">
            Watchlist 
