...
export function SwipeCard({ card, isTop, offset, onSwipe }: any) {
  ...
  return (
    ...
    >
      <div className="relative h-[65%] bg-gray-50 flex items-center justify-center p-4" onClick={() => setImgIndex(i => (i + 1) % card.images.length)}>
        <img src={card.images[imgIndex]} className="w-full h-full object-contain pointer-events-none drop-shadow-md" alt="" />
        ...
        <div className="absolute bottom-4 flex gap-1.5">{card.images.map((_: any, i: number) => <div key={i} className={`h-1.5 rounded-full transition-all ${i === imgIndex ? "w-5 bg-blue-600" : "w-1.5 bg-gray-200"}`} />)}</div>
      </div>

      <div className="flex-1 p-5 flex flex-col justify-between bg-white">
        <div className="space-y-3">
          <div className="flex gap-2"><span className="text-[9px] font-black px-2 py-0.5 bg-blue-50 text-blue-600 rounded-full uppercase">{card.category}</span><span className="text-[9px] font-black px-2 py-0.5 bg-gray-100 text-gray-500 rounded-full uppercase">{card.grade}</span></div>
          <h3 className="font-bold text-lg leading-tight line-clamp-2 text-gray-800">{card.name}</h3>
        </div>
        <div className="flex justify-between items-end border-t border-gray-50 pt-4">
          <div><p className="text-[10px] font-bold text-gray-400 uppercase mb-1 tracking-wider">Current Bid</p><span className="font-black text-2xl text-blue-600">${card.currentBid.toFixed(2)}</span></div>
          <div className="flex items-center gap-1.5 px-3 py-1 bg-red-50 text-red-500 rounded-lg font-black text-[10px] uppercase"><Timer className="w-3 h-3" /> {timeLeft}</div>
        </div>
      </div>
    </motion.div>
  );
